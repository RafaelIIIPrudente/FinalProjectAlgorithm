// The function implements a weighted breadth-first search (BFS) algorithm to find the shortest path and its weight between two nodes in a graph.
function weightedBFS(graph, startNode, endNode) {
  const queue = [[startNode, 0]];
  const visited = new Set();
  const parents = new Map();
  const weights = new Map([[startNode, 0]]);
...
function reconstructPath(startNode, endNode, parents) {
  const path = [endNode];
...
const graph = new Map([
  ...
  [
    "Approtech",
    new Map([
      ["Senior High School Parking Lot", 41],
      ["CPU Birthing Center", 63],
    ]),
  ],
  [
    "Senior High School Parking Lot",
    new Map([
      ["Approtech", 41],
  ...
  [
    ...
    new Map([
      ["Gate 3", 108],
      ["Senior High School Parking Lot", 17],
  ...
  [
    ...
    new Map([
      ["Senior High School Parking Lot", 59],
...
let startValue = start.value.toString();
let end = document.getElementById("EndPoint");
let endValue = end.value.toString();

// Event listeners to capture changes in the start and end values
start.addEventListener("change", function () {
  startValue = start.value.toString();
...
// Function to compute the path and weight when the button is clicked
function makePath() {
  const startValue = startPointDropdown.value.toString();
  const endValue = endPointDropdown.value.toString();

  const { path, weight } = weightedBFS(graph, startValue, endValue);
  const resultPath = path.join(" --> ");

  console.log(resultPath);
  console.log(`${weight} meters`);

  // Update the result elements in the HTML document
  const resultPathElement = document.getElementById("resultPath");
  if (resultPathElement) {
    resultPathElement.textContent = `Path: ${resultPath}`;
  }

  const resultWeightElement = document.getElementById("resultWeight");
  if (resultWeightElement) {
    resultWeightElement.textContent = `Distance: ${weight} meters`;
  }
}
